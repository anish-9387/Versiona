...
const NewRepo = () => {
  ...
  const update = (field: "name" | "description") => (e: React.ChangeEvent<HTMLInputElement>) =>
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      setLoading(true);
      const { data } = await api.post<{ repository: Repository }>("/repository/create", {
        owner: user!._id,
        name: form.name.trim(),
      ...
      setError(
        axiosErr.response?.data?.message || "Failed to create repository."
      );
    } finally {
      setLoading(false);
    }
